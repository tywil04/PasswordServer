import * as crypto from "{{ .FrontendRoutesJS.crypto }}"
...
window.auth.signup = async () => {
  ...
  let response = await fetch("{{ .BackendRoutes.signup }}", {
    method: "POST",
    body: JSON.stringify({
  ...
  if (success) {
    window.location = "{{ .FrontendRoutes.signin }}"
...
window.auth.signin = async () => {
  ...
  let response = await fetch("{{ .BackendRoutes.signin }}", {
    method: "POST",
    body: JSON.stringify({
      Email: emailInput.value,
      MasterHash: masterHash,
  ...
  if (jsonResponse.Authenticated) {
    window.location = "{{ .FrontendRoutes.home }}"
  } else {
    window.location.reload()
  }
}
...
window.auth.signout = async () => {
  let response = await fetch("{{ .BackendRoutes.signout }}", {
    method: "DELETE",
  })
  let jsonResponse = await response.json()
